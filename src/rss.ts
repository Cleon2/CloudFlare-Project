...
function parseItems(xml: string, sourceName: string): RawArticle[] {
  ...
  while ((m = re.exec(xml)) !== null) {
    ...
    articles.push({ title, url: link.trim(), source: sourceName, author, pubDate, content });
  }
...
export async function fetchArticlesForInterests(interests: string[]): Promise<RawArticle[]> {
  // Collect sources for the chosen interests, deduped by URL
  ...
  const results = await Promise.allSettled(
    sources.map(async (src) => {
      ...
      try {
        ...
        // Take the 3 freshest items per feed
        return parseItems(xml, src.name).slice(0, 3);
      } catch {
        clearTimeout(timer);
  ...
  // Cap at 12 articles total to keep AI costs reasonable
